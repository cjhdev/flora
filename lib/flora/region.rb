  ...
  class Region
    ...
    def initialize(**settings)
      ...
      if settings[:channels] and not(settings[:channels].empty?)
        ...
        update_channels(settings[:channels])
    ...
    def self.sf_bw_to_rate_up(*sf_bw)
      self::UP_RATES[sf_bw]
    end
    ...
    # this is required so we can generate cflist for a join accept
    # after we discover what gateway the join request has come from
